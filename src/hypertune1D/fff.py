...
def main():
    ...
    client.configure_experiment(parameters=[
        ...
        bounds=(steps_til_0_min, steps_til_0_max),                                                        
        parameter_type="int",
    ),
    ...
    for _ in range(args.num_trials):
        ...
        for trial_index, parameters in trials.items():
            ...
            try:
                _frontier_so_far = client.get_pareto_frontier()
                with open(OUTPUT_FILE_PTH, "w") as _f:
                    json.dump({"frontier": make_json_serializable(_frontier_so_far),
                               "loss_msgs": per_trial_loss_msgs}, _f)
                print(f"[checkpoint] wrote frontier after trial {trial_index} -> {OUTPUT_FILE_PTH}", flush=True)
            except Exception as _e:
                print(f"[checkpoint] could not save after trial {trial_index}: {_e}", flush=True)
