...
def train_baseline_nnrg(model: WrapperForNNRG,
    ...
    loss_spec = LossSpec(
        ...
        file_name_fn=lambda lr_, steps_, cofe_, desc_: get_plain_model_filename(lr_, steps_, check_for_overfit_every=check_for_overfit_every, desc=desc_),
        extra_wandb_config={},
        should_break_on_overfit=lambda step: True,
        
    )
    return run_training_loop(model, dataloader.array.shape[1], dataloader, loss_key, lr, dataset_test, directory_model_saving, loss_spec, steps, weight_decay=weight_decay, print_every=print_every, check_for_overfit_every=check_for_overfit_every, desc=desc, save_every=save_every)
...
def main():
    ...
    def get_description_of_job():
        return get_unique_identifier(vars(args)) + f"lsize{LATTICE_SIZE_ISING}"
    ...
    def make_and_save_visualizations_of_best_models(frontier, key_frontier, test_dataset):
        ...
        for i, (parameters, metrics, trial_index, arm_name) in enumerate(frontier):
            ...
            name_of_model = get_plain_model_filename(parameters[LR_PARAM_NAME], args.steps, args.check_overfit_every, get_description_of_job())
            ...
            fig, stats = compare_model_vs_validation(configs_sampled_from_model, configs_from_test_dataset, n_show=12)
            fname = "OutputVisplain" + get_plain_model_identifier(parameters[LR_PARAM_NAME], args.steps, args.check_overfit_every, get_description_of_job()) + ".pdf"
    ...
    for i in range(args.num_trials):
        ...
        for trial_index, parameters in trials.items():
            ...
            nrg_model, (_, loss_msgs) = train_baseline_nnrg(nrg_model, dataloader, loss_key,
                                                            parameters[LR_PARAM_NAME], validation_dataset, desc=get_description_of_job(),
                                                            directory_model_saving=model_saving_dir,
                                                             steps=args.steps, check_for_overfit_every=args.check_overfit_every,)
            ...
            nll_metric = float(just_nll(nrg_model, test_dataset))
            raw_data = {NLL_METRIC_NAME: nll_metric, MMD_METRIC_NAME: float(sample_quality),}
            metric_path = os.path.join(OUTPUT_FILE_SUBDIR, get_plain_model_identifier(parameters[LR_PARAM_NAME], args.steps, args.check_overfit_every, get_description_of_job()) + "__" + "metrics.json")
